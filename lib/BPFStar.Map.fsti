(* BPFStar.Map -- BPF map types and operations.
   ...
   permissions. Map lookup returns option, forcing null checks.
   Map value pointers are borrowed from the map and must be
   released before the programme returns.

   The specifications model the kernel's contract:
   - lookup may return None (key not found) or Some ptr
   - the returned pointer borrows from the map (map_value_perm)
...
(* Permission for a borrowed map value pointer. Returned by
   lookup and must be consumed (by reading, writing, or
   releasing) before the map can be used again.
   
   The ghost value tracks what we know about the map entry. *)
val map_value (#kt #vt: Type0) (m: bpf_map kt vt) (p: ref vt) (v: erased vt) : slprop

(* Look up a key in a BPF map.

   If the key exists, returns Some with a pointer to the value
   in the map. The caller gets map_value permission -- a borrow
   from the map. If the key does not exist, returns None.

   The map_perm is preserved across the call -- the map itself
   is not consumed. *)
val bpf_map_lookup_elem
  ...
  : stt (option (ref vt))
    ...
    (ensures fun r ->
      map_perm m **
      (match r with
       | Some p -> exists* v. map_value m p v
       | None -> emp))
...
val read_map_value
  ...
  : stt vt
    (requires map_value m p v)
    (ensures fun x -> map_value m p v ** pure (x == reveal v))
